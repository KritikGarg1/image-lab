...
img_gray = cv2.imread('img1.png',0)
...
for lpType in range(3):
  ilpFilter = createHPFilter(img_fft.shape, maxLoc, 30, lpType)
  ...
  cv2.imshow(name[lpType]+" Spectrum",gray_spectrum.astype('uint8'))
  cv2.waitKey(0)
  cv2.imshow(name[lpType]+" Result",ori_img)
  cv2.waitKey(0)
